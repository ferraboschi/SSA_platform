...
## Stato implementazione · port Next.js (App Router)

> Aggiornato al completamento del port. Stack: Next.js 16 (App Router, Turbopack),
> React 19, Tailwind v4, TS strict. DataSource in-memory dietro interfaccia
> astratta (swap verso Supabase/Render senza toccare le pagine). Verifica:
> `tsc --noEmit` 0 errori · `eslint` 0 problemi · tutte le route 200.

| Sez. | Modulo | Stato port | Sorgente |
|---|---|---|---|
| 0 | Fondazioni (tokens, UI, i18n, auth/ruoli, persistenza) | ✅ | `src/styles/`, `src/components/ui/`, `src/lib/i18n/`, `src/lib/auth/` |
| 1 | Shell & navigazione | ✅ | `src/components/shell/`, `src/app/(app)/layout.tsx` |
| 2 | Dashboard | ✅ | `src/components/dashboard/`, `src/app/(app)/dashboard/` |
| 3 | Corsi — catalogo + dettaglio | ✅ | `src/components/corsi/`, `src/app/(app)/corsi/` |
| 4 | Pianificatore | ✅ | `src/components/pianificatore/`, `src/app/(app)/pianificatore/` |
| 5 | Esami & test (+ live, studente, report) | ✅ | `src/components/esami/`, `src/app/(app)/esami/`, `src/app/(fullscreen)/` |
| 6 | Template materiali (CRUD persistito) | ✅ | `src/components/template-materiali/`, `src/lib/data/template-actions.ts` |
| 7 | Archivio | ✅ | `src/components/archivio/`, `src/app/(app)/archivio/` |
| 8 | Corsisti (lista + dossier) | ✅ | `src/components/corsisti/`, `src/app/(app)/corsisti/` |
| 9 | Educator (+ abilitazioni persistite) | ✅ | `src/components/educator/`, `src/app/(app)/educator/` |
| 10 | Account (profilo + switch ruoli) | ✅ | `src/components/account/`, `src/app/(app)/account/` |

**Trasversali richiesti**
- **Integrazioni** presenti come seam astratti (stub→live via env): Shopify, Airtable,
  Dropbox, Resend, Supabase — `src/lib/integrations/`. Tutta la mail passa da `EmailService`.
- **RAG** per correzione assistita esami: pipeline `src/lib/rag/` (chunking/embeddings/
  store/grading) documentata in `docs/rag-pipeline.md`.
- **Notifiche** a tipi-evento estensibili (registry) + invio Resend localizzato:
  `src/lib/notifications/`.
- **Multilingua** IT+EN completi, JA fallback `DeepPartial`, FR predisposto: `src/lib/i18n/dictionaries/`.

**Problemi noti del prototipo — risolti nel port**
1. Route `esame-studente` ora presente → `src/app/(fullscreen)/esame-studente/[id]`.
2. `Field` ridefinito nel render: i sub-componenti sono hoisted a livello di modulo (no remount/perdita focus).
4. Email Resend non più solo mockata: seam reale `EmailService` (stub logga in dev, live con `RESEND_API_KEY`).
