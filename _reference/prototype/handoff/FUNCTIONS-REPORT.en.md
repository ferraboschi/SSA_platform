...
## Implementation status · Next.js port (App Router)

> Updated on port completion. Stack: Next.js 16 (App Router, Turbopack),
> React 19, Tailwind v4, TS strict. In-memory DataSource behind an abstract
> interface (swap to Supabase/Render without touching pages). Verification:
> `tsc --noEmit` 0 errors · `eslint` 0 problems · all routes 200.

| Sec. | Module | Port status | Source |
|---|---|---|---|
| 0 | Foundations (tokens, UI, i18n, auth/roles, persistence) | ✅ | `src/styles/`, `src/components/ui/`, `src/lib/i18n/`, `src/lib/auth/` |
| 1 | Shell & navigation | ✅ | `src/components/shell/`, `src/app/(app)/layout.tsx` |
| 2 | Dashboard | ✅ | `src/components/dashboard/`, `src/app/(app)/dashboard/` |
| 3 | Courses — catalog + detail | ✅ | `src/components/corsi/`, `src/app/(app)/corsi/` |
| 4 | Planner | ✅ | `src/components/pianificatore/`, `src/app/(app)/pianificatore/` |
| 5 | Exams & tests (+ live, student, report) | ✅ | `src/components/esami/`, `src/app/(app)/esami/`, `src/app/(fullscreen)/` |
| 6 | Material templates (persisted CRUD) | ✅ | `src/components/template-materiali/`, `src/lib/data/template-actions.ts` |
| 7 | Archive | ✅ | `src/components/archivio/`, `src/app/(app)/archivio/` |
| 8 | Students (list + dossier) | ✅ | `src/components/corsisti/`, `src/app/(app)/corsisti/` |
| 9 | Educators (+ persisted qualifications) | ✅ | `src/components/educator/`, `src/app/(app)/educator/` |
| 10 | Account (profile + role switch) | ✅ | `src/components/account/`, `src/app/(app)/account/` |

**Required cross-cutting**
- **Integrations** present as abstract seams (stub→live via env): Shopify, Airtable,
  Dropbox, Resend, Supabase — `src/lib/integrations/`. All mail flows through `EmailService`.
- **RAG** for assisted exam grading: pipeline `src/lib/rag/` (chunking/embeddings/
  store/grading), documented in `docs/rag-pipeline.md`.
- **Notifications** with extensible event types (registry) + localized Resend dispatch:
  `src/lib/notifications/`.
- **Multilingual** full IT+EN, JA `DeepPartial` fallback, FR ready: `src/lib/i18n/dictionaries/`.

**Known prototype issues — resolved in the port**
1. `esame-studente` route now present → `src/app/(fullscreen)/esame-studente/[id]`.
2. `Field` redefined during render: sub-components are hoisted to module scope (no remount/focus loss).
4. Resend email no longer mock-only: real `EmailService` seam (stub logs in dev, live with `RESEND_API_KEY`).
